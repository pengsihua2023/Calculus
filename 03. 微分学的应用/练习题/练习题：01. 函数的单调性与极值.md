

# 📘 函数的单调性与极值（Monotonicity and extrema of functions）练习题与详解


## 题目 1

求函数 $f(x)=x^3-3x$ 的单调区间（Monotonic intervals）与极值（extremum）。

### 解答

1. 求导（Find the derivative）：

$$
f'(x)=3x^2-3=3(x^2-1)=3(x-1)(x+1)
$$

3. 驻点（Outpost）：

$$
f'(x)=0 \quad \Rightarrow \quad x=\pm 1
$$

4. 分区间讨论（Discuss each interval separately）：

* $(-\infty,-1)$： $f'(-2)=9>0$ → 递增；
* $(-1,1)$： $f'(0)=-3<0$ → 递减；
* $(1,+\infty)$： $f'(2)=9>0$ → 递增。

4. 极值：

* $x=-1$：极大值 $f(-1)=2$；
* $x=1$：极小值 $f(1)=-2$。

**答：** 递增区间 $(-\infty,-1)\cup(1,+\infty)$；递减区间 $(-1,1)$；极大值 $2$，极小值 $-2$。

---

## 题目 2

判断函数 $f(x)=x^4-4x^2$ 的单调区间（Monotonic intervals）与极值。

### 解答

1. 导数（derivative）：

$$
f'(x)=4x^3-8x=4x(x^2-2)
$$

2. 驻点（Outpost）：

$$
f'(x)=0 \quad \Rightarrow \quad x=0,\pm \sqrt{2}
$$

3. 分区间：

* $(-\infty,-\sqrt{2})$： $f'<0$ 递减；
* $(-\sqrt{2},0)$： $f'>0$ 递增；
* $(0,\sqrt{2})$： $f'<0$ 递减；
* $(\sqrt{2},+\infty)$： $f'>0$ 递增。

4. 极值（extremum）：

* $x=-\sqrt{2}$：极小值 $f(-\sqrt{2})=-4$；
* $x=0$：极大值 $f(0)=0$；
* $x=\sqrt{2}$：极小值 $f(\sqrt{2})=-4$。

---

## 题目 3

判断函数 $f(x)=\ln(x^2+1)$ 的单调性（Monotonicity）。

### 解答

1. 导数：

$$
f'(x)=\frac{2x}{x^2+1}
$$

2. 符号分析：

* $x>0$ 时， $f'(x)>0$ → 递增（increasing）；
* $x<0$ 时， $f'(x)<0$ → 递减（decreasing）。

**答：** 单调递减区间（Monotonically decreasing interval） $(-\infty,0)$，单调递增区间（Monotonically increasing interval） $(0,+\infty)$。

---

## 题目 4

求函数 $$f(x)=x^3-6x^2+9x+1$$ 的极值。

### 解答

1. 导数：

$$
f'(x)=3x^2-12x+9=3(x-1)(x-3)
$$

2. 驻点： $$x=1,3$$

3. 符号：

* $(-\infty,1)$： $f'>0$；
* $(1,3)$： $f'<0$；
* $(3,+\infty)$： $f'>0$。

4. 极值：

* $x=1$：极大值 $f(1)=5$；
* $x=3$：极小值 $f(3)=1$。

---

## 题目 5

已知 $f(x)=x-\ln(1+x)$，求其单调性。

### 解答

1. 导数：

$$
f'(x)=1-\frac{1}{1+x}=\frac{x}{1+x}, \quad (x>-1)
$$

2. 分析：

* 当 $x>0$， $f'(x)>0$ → 递增；
* 当 $-1<x<0$， $f'(x)<0$ → 递减。

---

## 题目 6

利用二阶导数判别法，求 $$f(x)=x^4$$ 在 $x=0$ 处是否有极值。

### 解答

1. $$f'(x)=4x^3 \quad \Rightarrow \quad f'(0)=0$$
2. $$f''(x)=12x^2 \quad \Rightarrow \quad f''(0)=0$$ → 不能直接判定。

邻域分析：

* $x\ne 0$ 时， $f(x)>0$；
* $f(0)=0$；
  因此 $x=0$ 为极小值。

---

## 题目 7

判断函数 $$f(x)=\sin x+\cos x$$ 在区间 $$[0,2\pi]$$ 的极值。

### 解答

1. 导数：

$$
f'(x)=\cos x-\sin x
$$

2. 解 $f'(x)=0$：

$$
\cos x=\sin x \quad \Rightarrow \quad x=\frac{\pi}{4}, \frac{5\pi}{4}
$$

3. 二阶导数：

$$
f''(x)=-\sin x-\cos x
$$

* $f''(\tfrac{\pi}{4})=-\sqrt{2}<0$ → 极大值 $f(\tfrac{\pi}{4})=\sqrt{2}$；
* $f''(\tfrac{5\pi}{4})=\sqrt{2}>0$ → 极小值 $f(\tfrac{5\pi}{4})=-\sqrt{2}$。

---

## 题目 8

求 $$f(x)=x e^{-x}$$ 的单调区间与极值。

### 解答

1. 导数：

$$
f'(x)=e^{-x}(1-x)
$$

2. 分析：

* $x<1$： $f'(x)>0$，递增；
* $x>1$： $f'(x)<0$，递减。

3. 极值：
   $f(1)=\frac{1}{e}$ 为极大值。

---

## 题目 9

判断函数 $f(x)=\frac{x}{x^2+1}$ 的单调性与极值。

### 解答

1. 导数：

$$
f'(x)=\frac{(x^2+1)-2x^2}{(x^2+1)^2}=\frac{1-x^2}{(x^2+1)^2}
$$

2. 符号：

* $(-\infty,-1)$： $f'<0$；
* $(-1,1)$： $f'>0$；
* $(1,+\infty)$： $f'<0$。

3. 极值：

* $x=-1$：极小值 $$f(-1)=-\tfrac{1}{2}$$；
* $x=1$：极大值 $$f(1)=\tfrac{1}{2}$$。

---

## 题目 10 （应用题）

在圆 $x^2+y^2=4$ 内，求矩形面积的最大值（矩形中心在原点，边平行于坐标轴）。

### 解答

矩形顶点为 $(x,y)$，面积

$$
S=4xy, \quad x^2+y^2=4
$$

1. 化约：

$$
y=\sqrt{4-x^2}, \quad S=4x\sqrt{4-x^2}
$$

2. 导数：

$$
S'(x)=\frac{16-8x^2}{\sqrt{4-x^2}}
$$

解得 $x=\pm \sqrt{2}$。

3. 最大面积：

$$
S_{\max}=4\cdot \sqrt{2}\cdot \sqrt{2}=8
$$

**答：** 最大面积为 $8$。

---

